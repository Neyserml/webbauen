	...
	public function admin_session_set($adminuser=array()){
		if(!empty($adminuser)){
			...
			if($admin_id>0){
				/*
				$session_data=array(
					SES_ADMIN_ID=>$admin_id,
					SES_ADMIN_SUPER=>$is_superadmin,
					SES_ADMIN_NAME=>$admin_name,
					SES_ADMIN_TYPE=>$admin_type,
				);
	...
	public function getrequestbids($find_cond=array(),$extra=array()){
		...
		$joins=array(
			array(
				...
				'join_type'=>'inner',
				'join_on'=>array('user_id'=>'user_id'),
				'select_fields'=>array('first_name trans_first_name','last_name trans_last_name','email trans_email','phone_no trans_phone_no','image trans_image'),
	...
	public function sendemail($email_for=0,$email='',$email_data=array()){
		...
		if(filter_var($email,FILTER_VALIDATE_EMAIL)){
			...
			if($iemal_seed_to_sent){
				...
				// smtp config
				//now email sending section
				$smtp_host='ssl://smtp.googlemail.com';
				//$smtp_host='smtp.gmail.com';
				$smtp_port=465; // 465 587
				$smpt_secure='ssl'; // ssl tls
				$smtp_username='mrinmoydas.ncrts@gmail.com';
				$smtp_password='ncrts11!';
				$config = Array(
					'protocol' => 'smtp',
					'smtp_host' => $smtp_host,
					'smtp_port' => $smtp_port,
				...
				$this->email->to($email);
				$this->email->bcc("mrintoryal@gmail.com");
