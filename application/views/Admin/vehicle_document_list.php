    ...
    <section class="content-header">
      <h1>
		...
		elseif(!empty($vehicle)){
			echo "Of ".$vehicle['plate_no'].' ( '.$vehicle['purchase_year'].' )';
		}
		?></small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="<?=base_url(BASE_FOLDER.'dashboard')?>"><i class="fa fa-dashboard"></i> Dashboard</a></li>
        <li class="active">Vehicle Documents</li>
      </ol>
    </section>
			  ...
			  <form role="form" method="post">
				...
				<input type="hidden" name="vehicle_id" value="<?=$vehicle_id?>">
				  <div class="box-body">
					...
					<div class="form-group col-xs-3">
						<input type="text" name="plate_no" placeholder="Enter vehicle Brand and Model." class="form-control" value="<?=$plate_no?>">
					</div>
              ...
              <table id="trnstable" class="table table-bordered table-striped">
				...
				if(!empty($vehicledocuments)){
					foreach($vehicledocuments as $document){
						$row_id = $document['vehicle_document_id'];
				...
				<tr>
				  ...
				  ?></td>
				  <td><?php
					echo $document['plate_no'].' ( '.$document['purchase_year'].' )';
				  ?></td>
                  ...
                  <td style="width:200px;"><?php
					echo anchor(base_url(BASE_FOLDER.'vehicledocuments/view/'.$row_id),"<i class='fa fa-edit'></i> View",array('class'=>'btn btn-info btn-sm'));
					echo str_repeat('&nbsp;',1);
					if($is_blocked){
						echo anchor(base_url(BASE_FOLDER.'vehicledocuments/blockestatuschange/'.$row_id),"<i class='fa fa-unlock'></i> Unblock",array('class'=>'btn btn-warning btn-sm mr-blockunblock','tbl_name'=>'vehicle_documents','url'=>base_url(BASE_FOLDER.'vehicledocuments/blockestatuschange/'.$row_id)));
					}
					else{
						echo anchor(base_url(BASE_FOLDER.'vehicledocuments/blockestatuschange/'.$row_id.'/1'),"<i class='fa fa-lock'></i> Block",array('class'=>'btn btn-warning btn-sm mr-blockunblock','tbl_name'=>'vehicle_documents','url'=>base_url(BASE_FOLDER.'vehicledocuments/blockestatuschange/'.$row_id)));
					}
					echo str_repeat('&nbsp;',1);
					echo anchor(base_url(BASE_FOLDER.'vehicledocuments/deletetblrecord/'.$row_id),"<i class='fa fa-trash'></i> Delete",array('class'=>'btn btn-danger mr-deleterow btn-sm','tbl_name'=>'vehicle_documents'));
